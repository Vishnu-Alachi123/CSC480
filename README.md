...
A Texas Hold'em AI project built on `pypokerengine`. Includes a Monte Carlo win probability engine, opponent behavior tracking, pot odds analysis, and a benchmark suite of agents showing how each feature contributes to performance.
...
# All 9 agents against each other (30 games)
...
# Other matchups: "components" (4 key agents) or "top" (4 strongest agents)
python -m src.tests.benchmark_agents top 50
```

Saves a chart to `benchmark_results_<mode>.png` (e.g. `benchmark_results_simple.png`).
...
Each agent adds exactly one feature on top of the previous one, so you can see what each part contributes.
